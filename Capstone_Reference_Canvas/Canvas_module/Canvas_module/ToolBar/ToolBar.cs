	...
	public partial class ToolBar : UserControl, IObserver
	{
		...
		private DrawObjectType GetDrawObjectType(string buttonName)
		{
			...
			switch (buttonName) 
			{
				...
				//case "HighLightPen": return DrawObjectType.HighLightPen;
		...
		private void HighLightPentoolStripButton_Click(object sender, EventArgs e)
		{
			// 펜의 크기만 조절 함
			SetToolBarButtonState(((ToolStripButton)sender).Text);
			Controller.MainController.Instance.LastUsedPenWidth = 12;
			Controller.MainController.Instance.LastUsedColor = Color =
				Color.FromArgb(
					50,
